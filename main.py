...
def main(args):
    ...
    emb_model = GNN(gnn_type = 'gin', num_class = 6, num_layer = args.num_layer, emb_dim = args.emb_dim, drop_ratio = args.drop_ratio, virtual_node = False).to(device)
    classifier = MLP([args.emb_dim, 6]).to(device)
    model = CompleteModel(emb_model, classifier)
